...
#  - [Console]::OutputEncoding: PowerShell console output variable
#  - $OutputEncoding:          PowerShell pipeline encoding
#  - chcp 65001:                actual console code page (the bit Python / Node / Vite inherit)
# Without chcp, child processes (uvicorn, vite) still default to GBK/cp936 on Chinese Windows,
# so their UTF-8 stdout is mis-decoded by the parent job host -> mojibake.
try {
...
$backendJob = Start-Job -Name 'backend' -ScriptBlock {
    ...
    # Start-Job spawns a brand new powershell.exe that does NOT inherit the
    # parent's chcp. We have to set UTF-8 codepage here too, otherwise uvicorn
    # writes GBK and the parent decodes garbage. PYTHONIOENCODING is belt-and-
    # braces in case chcp fails (some sandboxes lock it down).
...
$frontendJob = Start-Job -Name 'frontend' -ScriptBlock {
    ...
    # Same codepage fix as backend job: chcp 65001 so vite/Node see UTF-8.
    try { & cmd /c "chcp 65001 >nul" 2>$null } catch {}
    [Console]::OutputEncoding = New-Object System.Text.UTF8Encoding $false
    $OutputEncoding           = New-Object System.Text.UTF8Encoding $false
    $PID | Out-File -FilePath $pidFile -Encoding ascii -Force
    # NO_COLOR is the cross-tool way to disable ANSI escapes. PowerShell 5.x
    # host can't render ANSI colors anyway; this kills the [39m garbage
    # without losing the useful text content.
    $env:NO_COLOR = '1'
    Set-Location $dir
